...
| `recency` | measured | `status/inventory.yaml` | `archived`, `live`, `quiet`, `cold`, `unknown` |
...
The convention was already in this corpus — `status/harness.yaml`'s reading
...
3. **`ci/workspace-private.yaml` had two references swapped** against the
   ...
   roster and the same reference in `status/governance.yaml` denoted different
