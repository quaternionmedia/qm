...
project's own dedicated branch (`project/loopwall`, created from `main`) —
not copied into loopwall's own git history. The branch's ancestry *is*
...
plan (`PLAN.md`, `ROADMAP.md` in the loopwall repo) and any ADRs being
touched.
...
mismatch) runs from `project-seed/ci/adr-lint.yml`, ported into loopwall's
own `.github/workflows/adr-lint.yml` (submodule-aware: it checks out
`governance/qm` and lints `governance/qm/adr/DRAFT-*.md`, i.e. this
directory). The license gate required by the org open-license record is a
dependency-manifest-plus-allowlist check here, not an SBOM-per-image gate —
loopwall's runtime is a single host process, not a container or server
fleet: `uv.lock` is the manifest, and CI fails on any newly introduced
dependency whose license isn't OSI-approved/FSF-free (see
`DRAFT-dependency-disposition.md`, which already names the one deliberate
copyleft exception — FFmpeg/x264 — and why it's accepted).
