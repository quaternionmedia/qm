...
| The two `AGENTS.md` files are still hand-copied | `project-seed/ide/AGENTS.md` is a real file, not a symlink, and carries the discipline in adapted wording. `ci/check_discipline_parity.py` now fails when a rule is in one and not the other, which makes the drift detectable rather than impossible. Done: both regions generated from one source under the registry decision, and the parity check retired as redundant |
