...
|---|---|---|
| [`families-delineated.md`](families-delineated.md) | **a person, for one merge and two moves** — every reader of the estate now delineates by family (`qm estate --by-family`, a family column and a **By family** section in both harness views, a cookbook that resolves clones through the roster); the games family's private member is a reference, two project branches the roster could not see are rostered, and `private-names --source host` is clean. What remains: the pull request is merged by a person, two Godot-adjacent clones move under `repos/qm/` once the editor releases them, and the two new entries want a family claim once governance has run | qm |
