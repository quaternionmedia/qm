...
|---|---|---|
| [`enact-the-stages.md`](enact-the-stages.md) | **a human, for two acts in order** — clear every slot, then push `test`, which the stage record says is the enactment. Carries the re-keying inventory (every line that decides on `main`, read at `test`), the two `AGENTS.md` sentences that become the trap on that day, and the cross-tool findings: every named tool now reads `AGENTS.md` natively, the `CLAUDE.md` symlink is nine bytes on a Windows clone without `core.symlinks`, and one policy page names an adapter's slash commands | every repo |
| [`the-games-family-and-titanharvest.md`](the-games-family-and-titanharvest.md) | **a human, for two decisions** — whether `evolve/games-family` folds into the `test` train, and whether `quaternionmedia/titanharvest` is created. Carries the finding that the whole family estate has one copy, on `test`, and that the two family generators wrote CRLF on Windows | qm + titanharvest |
| [`the-estate-has-one-copy.md`](the-estate-has-one-copy.md) | **a human, for one step only** — a coordinated branch cleanup across five repositories, in the order that matters: preserve, then delete. The finding is not clutter but that a large body of work has exactly one copy and the oldest has had one copy for nineteen days, including the propagation fixes that themselves never propagated. Carries the two read-only views this needed and did not have: `qm branches` counts what deleting a branch would cost by patch rather than by commit, and `qm inbound` reads the upward channel the projects had been writing into their records all along | every repo |
| [`the-menu-contract-nobody-could-fetch.md`](the-menu-contract-nobody-could-fetch.md) | **a human, and one question only** — is rad's local-only window still open? Seven commits have sat on one disk since 2026-08-09, and two other repositories already cite documents and pin a contract version that only those commits carry. Also carries this session's rad/dossier/qm work, all unpushed, and a contradiction the nine-cells record could not survive being made executable | rad + dossier + qm + codecartographer |
...
| [`dossier-delta-review.md`](dossier-delta-review.md) | step 3 of that path. **Both stated blockers are gone**: the two alembic heads were reconciled and #12 is merged, so the slot is free. What the page still asks for has not been re-read against the tree | dossier |
