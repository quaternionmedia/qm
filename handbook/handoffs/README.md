...
|---|---|---|
| [`families-delineated.md`](families-delineated.md) | **a person, for two family claims** — every reader of the estate now delineates by family (`qm estate --by-family`, a family column and a **By family** section in both harness views, a cookbook that resolves clones through the roster); the games family's private member is a reference, two project branches the roster could not see are rostered, and `private-names --source host` is clean. Landed as #114; every Godot-adjacent clone is under `repos/qm/`. What remains: the two new entries want a family claim once governance has run, and two checks are named and unbuilt | qm |
