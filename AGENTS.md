...
15. **An exit status is read from the command, never through a pipe.**
    ...
    itself, or `PIPESTATUS`. This has turned a failing check into a reported
    pass three times here, most recently on a `REFUSED` verdict printed
    directly above `exit=0`. The same care for anything fetched: **re-fetch a
...
16. **A bound stated in prose is not a bound.** Item 13 assumes the guard
    ...
    reads as done and holds nothing: a slot exemption shipped here with its own
    precondition stated in its docstring and enforced nowhere, and eighteen of
    eighteen local gates passed over it.
    **And a relaxation needs the adversarial pass more than a restriction
    ...
    ever: nobody files a report about a check that let them through. Both of
    that session's real holes were in things that permitted something.

17. **A summary of a canonical list is a copy, and copies drift.** Both
    ...
    relation. This file listed three of the five branch namespaces the README
    defined, through the rewrite that touched it most heavily. Either generate
    the summary from its source or mark it derived and name that source in the
    text. A hand-maintained restatement carrying no pointer is the one that
