...
were never published — and that drift was observed in this project's own
first drafting round. The rules below make the discipline mechanical instead
...
This project is the reference instance of the QM constitution: the pin above
is set when the org records ratify. On ratification of the org open-license
record, ADR-0001 receives a dated amendment recording adoption-by-reference
(body untouched).
...
| # | Title | Status | Date |
|---|---|---|---|
| 0001 | Open-license exclusion and upstream-contribution remediation | Accepted | 2026-06-09 |

Drafts in flight (numberless, by title): media router; VOD/library layer;
control-plane seam (build vs buy); publish auth via HTTP delegation;
postprocess DAG; recording & packaging formats; playback & latency tier
*(pends: latency decision)*; source-type abstraction; camera ingest
*(pends: cameras in scope?)*; multi-party composition; cold-storage tier;
deployment & provenance.

Org-level philosophies (seams doctrine, build-the-seam doctrine, house
stack, contribution & sponsorship, decision-record discipline) live in the
constitution's `records/`, not here.
