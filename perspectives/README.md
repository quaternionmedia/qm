...
| 2026-07-18 | `claude-sonnet-5-2026-07-18-test-timeout-halting-problem-retrospective.md` | Claude Sonnet 5 | Perspective | Unreviewed | Process retrospective on an unbounded-test-watching incident; `adr/DRAFT-enforced-test-timeouts.md` and `CONTRIBUTING.md`'s testing section are the concrete responses it discusses. The underlying test bug itself (left open at time of writing, §4) was resolved same session: `LayoutToggleVideoTest.kt`'s "unit symbols" case now scrolls to an adjacent already-working node (`symbol_only_controls_switch`) rather than using the actual target as the `performScrollTo()` target itself, which reproducibly hung. |
