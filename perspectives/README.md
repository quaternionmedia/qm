...
| 2026-08-15 | `2026-08-15-stating-a-constraint-is-not-enforcing-it.md` | Peter Kagstrom | Perspective | Unreviewed | Tools: Claude Opus 5, in the session that produced every error it lists. A slot exemption shipped with its own precondition stated in the docstring and enforced nowhere, found by re-review after 18/18 gates passed; three placeholders introduced for one namespace by the session that had just written a perspective about that failure mode; and three questions handed to a reviewer that the corpus already answered. Companion handoff: `handbook/handoffs/exploration-namespace.md` |
