...
| 2026-08-15 | `2026-08-15-a-namespace-with-one-direction.md` | Peter Kagstrom | Perspective | Unreviewed | Tools: Claude Opus 5. `workspace/math-experiments` sat 74 commits behind `main` with no `.github/` directory, because the namespace's rules constrained one direction of flow and the other was never written. The same incompleteness is already diagnosed one paragraph below the README's own namespace table, for `propagate/*`. Facts, no Status implied: the standard is drafted on `evolve/exploration-branch-namespace` as `handbook/research-workspaces.md` |
